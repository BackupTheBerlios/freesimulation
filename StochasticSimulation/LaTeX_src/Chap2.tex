...
\begin{figure}[htbp]
  \begin{center}
    % \includegraphics[width=\textwidth]{Figures/StreamsConnect.eps}
...
with a stacktrace showing the last commands executed. This progragation
is a great simplification, because you can catch all of ythe exceptions
in one place and not distribvuted through the whole class hierarchy.
...
``Array out of Bounds'', or ``IO Exception'', etc.
...
\begin{table}[htbp]
  \begin{center}
    \begin{tabular}{ll}
      Exception name & Ocurrence \\\hline
      IOException & Some IO problem. \\
      ArrayIndexOutOfBoundsException & array is out of bounds \\
     ??? & ??? \\
    \end{tabular}
    \caption{Some of the important exceptions in the \texttt{java.lang.Throwable} class. A detailed list is in the API documentation.}
...
parent class of all of the classes and catch all the exceptions there.

If you ask how do you know what exceptions to catch, there are only
...
turns zero or step size/accuracy gets too large.
