...
Mathematica& Win95 & Mathematica V3.0 & Pentium 133 & 28 Min. \\\hline
    ...
    {Performance comparison for different languages, operating
      ...
      using the JIT use option \textt{-nojit} up to JDK1.1.7 on Windows or
...
suited for the heterogenous world of networked computers seen today.
...
especially Netscape which included a Java Virtual Machine (JVM) into
their widley used browser Netscape Navigator. Then in early 1997
...
Also many new APIs (Application Progammers Interfaces) have appeared, some
...
At the same time SUN announced a new project called JINI, which is
a ``small set of instructions and interfaces'' based on Java to
be used to drive and use arbitrary electronic devices --
the actual aim of the project started in 1991. 
  


\subsection{Useful Java tools (most of them 
        used for the programs in this book)}
\begin{description}
\item[Programs]
\begin{description}
\item[JDK] The Java Development Kit, distributed freely by SUN.\\
        ...
        for a Java compiler, virtual machine and a debugger. There
        ...
        command line to use it. There are no icons coming with the
...
\item[JDE] JDE (Java Development Environement) is an Emacs/Xemacs extension
        written in Elisp. \\
        It enables you to write your Java code much faster, if you are
        using the emacs or xemacs editors. 
\item[Netscape Navigator/Communicator] A web browser like the Internet 
        ...
        freely available. \\
        We used Natscape as a reference to check, if software is running
...
\item[Ptolmey (Ptplot)] A package to produce 2D plots from data. \\
        ...
        plot package, called Ptplot is currently fully funtional. We will
...
\item[JNL] The Java Numerical Library written by Visual Numerics and
        distributed freely\foornote{But please consult the License Agreement
        ...
        provides some more important standard functions (hyperbolic
...
There is no power operator like $**$ in FORTRAN or $^$ like in many
...
EINFUEGEN: Buffon needle procedural !!!!
...
\inputlisting{Listings_Java/Needle.java}
...
\subsubsection{Calculating Moments}
...
\inputlisting{Listings_Java/Moments_all.java}
, the ``traditional procedural'' approach
\inputlisting[23,28]{Listings_Java/Moments_procedural.java}
\inputlisting[35,42]{Listings_Java/Moments_procedural.java}
or an object oriented approach (two examples): 
\inputlisting[23,28]{Listings_Java/Moments_object.java}
\inputlisting{Listings_Java/MomentsData.java}
\inputlisting[25,33]{Listings_Java/Moments_object2.java}
\inputlisting{Listings_Java/MomentsData2.java}
...
we want to recapitualte the basic differences and features of
...
method, which has to be static and public. This is also of course the
...
To that purpose go ahead and write a simple program, which only 
...
as an applet and in a brwoser as an applet. What you will get is the
following:
\begin{figure}[htbp]
  \begin{center}
    \leavevmode
    \includegraphics{Figures/LineOfExecution.eps}
...
because we left it empty. The trick was that there is for an applet 
always another thread (see \ref{ParallelJava} for an introduction to threads)
running, which takes care of (re)painting the windows used by the applet.
This thread is a method from the AWT package (see \ref{AWTIntro}) and
...
method and that again calls the \verb|paint()| method, when there is the time
...
(\verb|appletviewer applet.java|) it executes the HTML code supplied
...
\end{verbatim}
You can start it (first compile it) 
by typing \verb|appletviewer test_applet.java| or 
in the Netscape Naviagtor type \verb|file:/home/user/test_Applet.java|.
...
\section{Exercises}
Pi Calculation:
\inputlisting{Listings_Java/Pi_Calc_plain.java}
