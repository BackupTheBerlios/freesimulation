...
Mathematica& Win95 & Mathematica V3.0 & Pentium 133 & 30 Min. \\\hline
    \end{tabular}
    \caption{Performance comparison for different languages, operating
...
For other conversions y<ou can use the \verb|Long.parseLong()| or the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Buffon needle problem}
...
\caption{The buffon needle problem. The $x$--$\phi$ plane.}
\end{figure}

Some years later (??) Laplace \footnote{Laplace, ...} recognized that
...
have also generic properties wich justify to define needles as a class!
...
cordinates which specify the position of the needle. 
...
or a local variable ina method have the same name as one of the fields
...
Next we need to look at the class \verb|nadel.java| which contains the
...
object. We now have to create dynamically the the object. This
...
As a second example for the most impiortant issues about objects, we
...
(JNL) and is free. It is alos proposed to become standard in the
nes Java language specification.
