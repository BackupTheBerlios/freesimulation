...
and in the examples of the next chapters we will need 
the definition of conditional probability densities.
...
\frac{\partial}{\partial t} \langle f(y) \rangle &=&
 ...
 &= & \lim_{\Delta t \rightarrow 0} \frac{1}{\Delta t}
       ...
       \right\}.
\end{eqnarray*}
Making use of the Chapman--Kolmogorov equation in the first term 
...
Thus, the time evolution equation for the expectation value can be written 
...
Multiplying equation (\ref{EQ_EXPECTATION}) 
...
\end{equation}
We will call the above equation the differential Chapman--Kolmogorov equation.
...
Obviously we have for the expecatation value
\begin{equation*}
Ef(x(t)) = f(\phi(x,t)),
\end{equation*}
...
\begin{equation*}
{\cal{A}}_L(t) f = \sum_i \frac{\partial}{\partial x_i}(x) g^i(x).
\end{equation*}
...
${\cal{A}}_L$. It is evident that we have
\begin{eqnarray}
\label{GEN_DET0}
...
statistical physics. In order to give an example of the occurrence
...
Following the above derivation the dynamics of this probability distribution
...
\begin{equation*}
i \frac{\partial}{\partial t} P^N =  {\cal{L}} P^N,
\end{equation*}
where
\begin{equation*}
...
Liouville equation may be solved  to find the probability density
at later times $t$
\begin{equation*}
...
the master equation describe the gain of state due to transitions 
...
\end{eqnarray*}
Thus we have found the following average of the stochastic 
variable $N(t)$
\begin{equation}
...
{\cal{A}}_{KM}(y) = \sum_{\nu=1}^{\infty}
   ...
   \right)^{\nu}.
\end{equation*}
It is clear that dealing with the Kramers--moyal expansion will 
not be easier then dealing with the original master equation.

A particularly interesting approximation to a jump process is 
...
\end{equation}
The above equation is called the Fokker--Planck equation.
...
motion, if observed on a time scale which is larger then the 
autocorrelation time of the velocity, i.e. many collisions have 
occurred between two observations, may be described by a Markov 
process. If we regard, for simplicity a one--dimensional 
situation, the Brownian particle makes random jumps back and forth 
along the $x$--axis. The jumps my have any length but the
probability for large jumps falls off rapidly. 
It is reasonable to assume that these probabilities are 
symmetrical and independent from the starting point.

Hence we have
\begin{equation*}
a_1 = \frac{\Delta X}{\Delta t} = 0
\end{equation*}
and
\begin{equation}
a_2 = \frac{(\Delta X)^2}{\Delta t}= \text{const}.
\end{equation}
...
\end{equation}
Hence, the width of the distribution increases witd the square 
