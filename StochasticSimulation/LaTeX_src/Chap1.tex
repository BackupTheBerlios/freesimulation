...
if you just prepend a capital \texttt{J} to all class names, you can %
...
\begin{table}[htbp]
  \begin{center}
    \begin{tabular}{c|p{0.6\textwidth}}
      ...
      TextComponent.TextField & A line of text, optionally editable. \\
      ...
      Container.Window.Dialog & A dialog window to ask a question or display
                                a notification. \\
      Container.Window.FileDialog & For choosing/browsing files/directories. \\
    \end{tabular}
    \caption{A list of all defined AWT components of the Java 1.1 API.}
...
\begin{table}[htbp]
  \begin{center}
    \begin{tabular}{c|l}
      ...
      GridBagLayout & A very versatile but complicated layout manager. \\
    \end{tabular}
...
in section \ref{sec:AWTAdvanced}. 
...
is borrowed from the \emph{pxgraph} program. It is like using GNUPlot with
...
can only handle 2D plots and JSci can handle some easy 3D plots.

There is also a program called 
\verb|SciVis|\footnote{\href{???}{???}|}
...
files, but first you would have to understand the data format.
The authors told us they are developing a Java interface to
supply data directly to \verb|SciVis|, but we have not heart any
news yet. 
...
This is a full implementation of threee dimensional routines to
produce all kinds of 3D scenes and objects and even move them, etc.
...
Now that we have learned to make plots in Java, it is of great
importance to get a printed version of our graphics. And even for
this Java has commands for initiating a print job and preparing
the output for the printer. Because the procedure has changed
...
\item Start ``printing'':
  ...
  \item Get the graphics context for the component in question. Use the
    \verb|getGraphics()| mehtod of the \verb|java.awt.PrintJob| class.
...
some editing om the postscript files. First of all you have to
add a bounding box line at the beginning of the postscript
file. You can do this by hand, put a line
\begin{verbatim}
...
interface. There has been introduced a new class: \verb|java.awt.print|.
This class provides much more sophisticated methods and it even
is able of handling color models, which is very important for
...
to initiate the printjob, allowed by the security manager. 
...
\begin{figure}
\label{FIG_DECAYA}
\includegraphics[width=10cm]{Figures/f_decaya.eps}
...
\begin{figure}
\label{FIG_DECAYB}
\includegraphics[width=10cm]{Figures/f_decayb.eps}
\caption{A realization of the stochastic process of radioactive decay.
